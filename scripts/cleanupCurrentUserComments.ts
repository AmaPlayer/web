  ...
  try {
    ...
    for (const comment of commentsToDelete) {
      try {
        ...
        console.error(`  ⚠️  Failed to delete comment ${comment.id}:`, error);
      }
    }
    ...
    for (const [momentId, count] of momentCommentCounts.entries()) {
      try {
        ...
        console.error(`  ⚠️  Failed to update moment ${momentId}:`, error);
      }
    }
  ...
  try {
    ...
    for (const postUpdate of postsToUpdate) {
      try {
        ...
        console.error(`  ⚠️  Failed to update post ${postUpdate.postId}:`, error);
      }
    }
