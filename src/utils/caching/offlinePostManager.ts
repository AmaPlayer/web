...
export class OfflinePostManager {
  ...
  async syncPendingFollows() {
    try {
      const pendingFollows = (await idbStore.getPendingFollows()) as any[];
      const pendingUnfollows = (await idbStore.getPendingUnfollows()) as any[];
      ...
      for (const follow of pendingFollows) {
        try {
          ...
          await userService.default.toggleFollow(follow.followerId, follow.followingId);
      ...
      for (const unfollow of pendingUnfollows) {
        try {
          const userService = await import('../../services/api/userService');
          await userService.default.toggleFollow(unfollow.followerId, unfollow.followingId);
