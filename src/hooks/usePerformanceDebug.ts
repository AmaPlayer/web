...
export function useGlobalPerformanceDebug(options: {
  ...
  useEffect(() => {
    ...
    if (consoleCommands && typeof window !== 'undefined') {
      ...
      console.log(
        '🔍 Performance debugging tools available at window.performanceDebug\n' +
        'Commands: start(), stop(), report(), clear(), export(), renderStats(), timerStats(), alerts()\n' +
        'Keyboard shortcut: Ctrl+Shift+P to toggle debug panel'
      );
    }
