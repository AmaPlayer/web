...
export function useRenderDebugger(
  ...
  const renderAnalysis = useMemo(() => {
    ...
    if (logToConsole && renderCountRef.current > 1) {
      console.group(`🔄 ${componentName} re-render #${renderCountRef.current}`);
      console.log('Reason:', renderReason);
    ...
    return () => {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.log(`📊 ${componentName} lifecycle:`, {
          ...
          renderCount: renderCountRef.current,
          averageRenderInterval: renderCountRef.current > 1 
            ? (totalLifetime / (renderCountRef.current - 1)).toFixed(2) + 'ms'
            : 'N/A'
        });
      }
    };
