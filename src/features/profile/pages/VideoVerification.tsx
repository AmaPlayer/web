...
const VideoVerificationPage: React.FC = () => {
  ...
  useEffect(() => {
    const loadVideoData = async () => {
      try {
        ...
        // Fetch user's talent videos from Firestore
        ...
        const talentVideos = userData.talentVideos || [];
        const targetVideo = talentVideos.find((v: TalentVideo) => v.id === videoId);
  ...
  const handleSubmitVerification = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updatedVideos = talentVideos.map((v: TalentVideo) => {
        if (v.id === videoId) {
          const currentVerifications = v.verifications || [];
          const updatedVerifications = [...currentVerifications, newVerification];
          const threshold = v.verificationThreshold || 3;

          // Auto-verify if threshold reached
          const newStatus = updatedVerifications.length >= threshold ? 'verified' : 'pending';

          // Clean the video object - remove undefined values
          return cleanObject({
            ...v,
        ...
        // Clean existing videos too - remove undefined values
